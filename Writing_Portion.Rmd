...
When students are first presented with Markov Chains, the game Monopoly often serves as a motivational example. The Monopoly board (pictured above) is a continuous Markov chain with an interesting property. When a player lands on the "Go to Jail" space, forward movement in the chain is interupted when the player is sent backwards to the "Jail" space. In the long-term probabilities of the board, this means that owning the properties between "Jail" and "Go to Jail" gives the player an advantage in gameplay.   

This type of analysis struck me as particularly interesting in my early study of probabilty. So, for this project, I decided to analyze a board game with the Monopoly problem in mind. After some Internet searching, I settled on The Game of Life, since I could not find a formal analysis of it. In what follows, I will construct a proability matrix for the board and create a simulation of the game with the goal of highlighting some of the interesting aspects of gameplay. 
...
In contructing a transition matrix, I made two assumptions:
...
\begin{enumerate}
  \item The players is assigned a Salary from one of two decks: College or Career. The average salary for the College deck is higher than the average salary for the Career deck. The player has two opportunites to obtain a college career. First, they may choose the College path from the starting position. Second, they may choose to follow the Night School path from space 38. These paths have an opportunity cost, since the paths for college careers are longer. The player recieves their salary each time a "Payday" space is passed or landed on. 
  ...
  \item Players retire. Each player recieves a bonus upon reaching retirement, with bonuses decreasing by place (i.e. first to finish recieves 400K, next recieves 300K, etc.).
\end{enumerate}
...
\begin{enumerate}
  \item The game spinner is unbiased, so spins $\in [1,10]$ are equally likely.
  \item At each path split, the player chooses their path randomly. The one place this assumption makes things funky is if a player has chosen the College path and then chooses the Night School path. It seems unlikely that someone would choose to pursue a College salary twice in real gameplay.
...
---
nocite: | 
  @dobrow_2016, @dobrow_2014, @hayes_2013, @hayes_2017
...
